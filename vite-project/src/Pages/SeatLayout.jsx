  ...
  const handleSeatClick =(seatId)=>{
    if(!selectedTime){
     return toast("please select the time first")
      ...
      return toast('we can only select 5 seats')
    }
...
<div key={row} className='flex gap-2 mt-2'>
  ...
  // Handle both cases: date as key OR as index
  const dateKeys = Object.keys(show.dateTime)
  const currentKey = dateKeys.includes(date) ? date : dateKeys[Number(date)]
  const timings = show.dateTime[currentKey] || []

  return (

    <>
      <div className="flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-10 md:pt-16 mt-44">
